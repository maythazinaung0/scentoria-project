  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="flex items-center gap-0.5">
            ...
            {user ? (
              <div className="hidden md:flex items-center gap-0.5">
                {isAdmin ? (
                  <Link
                    to="/admin"
                    className="text-[11px] text-nature-olive border border-nature-olive/40 px-3 py-1 rounded hover:bg-nature-olive hover:text-white transition-all duration-300 tracking-[0.15em]"
                  >
                    ADMIN
                  </Link>
                ) : (
              ...
              <Link
                to="/login"
                className="hidden md:block text-[11px] text-nature-charcoal hover:text-nature-olive tracking-[0.15em] transition-all duration-300 ml-2"
              >
                SIGN IN
              </Link>
            )}
            ...
            >
              <div className="relative w-5 h-5">
                <span className={`absolute left-0 w-5 h-[1.5px] bg-current transition-all duration-300 ${menuOpen ? 'top-2 rotate-45' : 'top-1'}`} />
                <span className={`absolute left-0 top-2 w-5 h-[1.5px] bg-current transition-all duration-300 ${menuOpen ? 'opacity-0 scale-0' : 'opacity-100 scale-100'}`} />
                <span className={`absolute left-0 w-5 h-[1.5px] bg-current transition-all duration-300 ${menuOpen ? 'top-2 -rotate-45' : 'top-3'}`} />
              </div>
            </button>
          </div>
        </div>
        ...
        <div
          className={`overflow-hidden transition-all duration-500 ease-out ${searchOpen ? 'max-h-16 opacity-100 pb-3' : 'max-h-0 opacity-0 pb-0'
            }`}
        >
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                onSearchChange(val);
                if (val.length > 0 && location.pathname !== '/products') {
                  navigate('/products');
                }
              }}
              className="w-full bg-nature-card/80 border border-nature-border rounded px-10 py-2.5 text-nature-dark placeholder-nature-muted focus:outline-none focus:border-nature-olive/50 text-sm transition-all duration-300"
            />
          </div>
        </div>
      </div>
