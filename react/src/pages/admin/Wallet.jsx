...
function TopupRow({ topup, onDecide, deciding }) {
    ...
    <>
      <div className="bg-white/30 backdrop-blur-xl border border-white/60 rounded-2xl p-5 shadow-[0_2px_16px_-4px_rgba(44,53,39,0.08)] hover:shadow-[0_16px_40px_-8px_rgba(74,104,56,0.15)] transition-shadow">
        <div className="flex items-start gap-4">
          <button
            onClick={() => topup.transaction_image_url && !imgError && setPreviewOpen(true)}
          ...
          >
            {topup.transaction_image_url && !imgError ? (
              <img
      ...
      {previewOpen && (
        <ImagePreviewModal url={topup.transaction_image_url} onClose={() => setPreviewOpen(false)} />
      )}
    </>
  );
}
    ...
    <div className="min-h-screen relative overflow-hidden selection:bg-nature-olive/10">
      <div className="relative p-6 md:p-10 z-10">
        <div className="text-nature-dark space-y-8 max-w-4xl mx-auto">
          ...
          {loading ? (
            <div className="flex items-center justify-center gap-2 text-nature-muted text-sm py-16">
