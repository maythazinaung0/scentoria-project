    ...
    <div className="fixed inset-0 bg-nature-dark/85 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className={`bg-nature-card border border-nature-olive/20 rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto ${HIDE_SCROLLBAR} space-y-5 animate-fadeIn`} onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-4 items-center bg-nature-bg p-3 rounded-xl border border-nature-olive/20">
          {showImage ? (
            <img
              ...
              className="w-24 h-24 rounded-xl object-cover border border-nature-olive/20"
            />
          ) : (
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {(() => {
                ...
                return (
                  <span className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${status === 'active' ? 'bg-emerald-100 text-emerald-800' : 'bg-red-100 text-red-700'}`}>
                    {status}
                  </span>
                );
              })()}
        ...
        <div className="grid grid-cols-2 gap-3 text-xs">
          <div className="bg-nature-bg border border-nature-olive/20 p-2.5 rounded-lg border border-nature-olive/15">
            <span className="text-nature-olive text-[10px] font-semibold uppercase tracking-wider block mb-0.5">Scent Family</span>
            <span className="font-medium text-sm text-nature-dark">{product.scent?.name ?? '—'}</span>
          </div>
          <div className="bg-nature-bg border border-nature-olive/20 p-2.5 rounded-lg border border-nature-olive/15">
          ...
          <div className="col-span-2 border border-nature-olive/20 bg-nature-bg p-2.5 rounded-lg border border-nature-olive/15">
            <span className="text-nature-olive text-[10px] font-semibold uppercase tracking-wider block mb-0.5">Slug Reference</span>
            <span className="font-mono text-nature-dark select-all">{product.slug}</span>
          </div>
          {product.description && (
            <div className="col-span-2 border border-nature-olive/20 bg-nature-bg p-3 rounded-lg border border-nature-olive/15">
              <span className="text-nature-olive text-[10px] font-semibold uppercase tracking-wider block mb-1">Description</span>
              <p className="text-nature-dark leading-relaxed font-normal">{product.description}</p>
            </div>
          )}
        </div>
